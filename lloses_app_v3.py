...
LANG = {
  "es":{
    ...
    "export_list":"⬇️ Exportar CSV","phone":"Teléfono","edit_neighbor":"Editar vecino",
    ...
    "new_year":"Crear nuevo año","import_csv":"📂 Importar vecinos desde CSV",
    "csv_format":"Columnas requeridas: nombre, direccion, email, telefono, iban",
  ...
  "ca":{
    ...
    "export_list":"⬇️ Exportar CSV","phone":"Telèfon","edit_neighbor":"Editar veí",
    ...
    "iban_hint":"ES00 0000 0000 0000 0000 0000",
    "new_year":"Crear nou any","import_csv":"📂 Importar veïns des de CSV",
    "csv_format":"Columnes necessàries: nombre, direccion, email, telefono, iban",
...
def init_db():
    with db() as con:
        ...
        con.execute("""INSERT OR IGNORE INTO tarifas VALUES
            (1,0.85,5.0,0.10,'Ajuntament de Les Lloses',
    ...
    try:
        ...
        for r in required:
            if r not in df.columns:
                return 0, f"Falta columna obligatoria: '{r}'"
        count=0
        with db() as con:
            for _,row in df.iterrows():
                ...
                # Insertar si no existe (por nombre)
...
def send_email(to_addr, nombre, nf, mes, anyo, pdf_bytes, from_addr, from_pass, lang="es"):
    """
    Envía email con adjunto PDF.
    Detecta automáticamente Gmail vs otros servidores.
    """
    L=LANG[lang]
    try:
        msg=MIMEMultipart()
        ...
        p.set_payload(pdf_bytes)
        encoders.encode_base64(p)
        p.add_header("Content-Disposition",f"attachment; filename={nf}.pdf")
        msg.attach(p)
...
# ══ ADMIN: VECINOS ════════════════════════════════════════════════════════════
    ...
    with t1:
        ...
        st.download_button(T("export_list"),data=csv,file_name="vecinos.csv",mime="text/csv")
    ...
    with t_csv:
        ...
        template=pd.DataFrame(columns=["nombre","direccion","email","telefono","iban"])
        template.loc[0]=["Joan Puig","Carrer Major 1","joan@exemple.com","600111001","ES00 0000"]
        st.download_button("⬇️ Descargar plantilla CSV",
            data=template.to_csv(index=False).encode(),
            file_name="plantilla_vecinos.csv",mime="text/csv")
        st.markdown("---")
        uploaded=st.file_uploader("Seleccionar archivo CSV",type=["csv"],key="csv_upload")
        if uploaded:
            df_prev=pd.read_csv(uploaded,dtype=str).fillna("")
...
# ══ ADMIN: FACTURACIÓN ════════════════════════════════════════════════════════
    ...
    if col_save.button("💾 Guardar configuración email"):
        with db() as con:
            con.execute("UPDATE tarifas SET email_from=?,email_pass=? WHERE id=1",(ef,ep))
        st.success(T("saved_ok"))
