    ...
    try:
        ...
        if smtp_host:
            ...
            if domain not in smtp_map:
                raise ValueError(
                    f"Dominio '{domain}' no reconocido. "
                    "Abre '⚙️ Configuración SMTP avanzada', rellena el Servidor SMTP, "
                    "Puerto y SSL, y pulsa 'Guardar configuración email' antes de enviar."
                )
            host,port,use_ssl=smtp_map[domain]
        # Contexto SSL
        if smtp_no_verify:
...
# ══ ADMIN: FACTURACIÓN ════════════════════════════════════════════════════════
    ...
    if col_send.button(T("send_all")):
        if not ef or not ep:
            ...
            # Releer config SMTP siempre desde BD para evitar valores stale
            tar_fresh=get_tar()
            ...
            for i,row in enumerate(amb):
                ...
                # Enviar email con config fresca de BD
                ok,err=send_email(row["email"],row["nombre"],nf2,mf,af,pdf_b,_ef,_ep,st.session_state["lang"],
        ...
        if row_s.get("tiene_email") and row_s.get("email"):
            if col_b.button(T("send_btn"),key="btn_send_single"):
                if not ef or not ep:
                    ...
                    ok,err=send_email(row_s["email"],row_s["nombre"],nf_s2,mes_s,af,pdf_s2,_ef,_ep,st.session_state["lang"],
                                    _host,_port,_ssl,_noverify)
